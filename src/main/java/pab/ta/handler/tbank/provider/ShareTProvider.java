...
public class ShareTProvider implements AssetInfoProvider {
    ...
    public List<AssetInfo> info() {
        ...
        try {
            return future.get()
                    .stream()
                    .filter(asset -> asset.getRealExchange().equals(RealExchange.REAL_EXCHANGE_MOEX) && asset.getClassCode().equals("TQBR"))
                    .map(asset -> new BaseAssetInfo(asset.getUid(), asset.getTicker(), AssetType.SHARE, asset.getName()))
